...
- En Playwright de tablas MUI/DataTable, no asumir que props `id` se renderizan en DOM (`#preparaciones-table`, `#preparaciones-tabs`); preferir `getByRole` y texto visible estable.
- En tests de permisos, no usar perfil `rol: 'admin'` cuando se quiere validar ocultacion de acciones por permiso porque puede existir bypass por rol.
