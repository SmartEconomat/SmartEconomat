...
let ArchivoService = class ArchivoService {
  async uploadFile(file, user) {
    if (!file) {
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('FILE_REQUIRED')
      );
    }
    let fileUrl = '';
  ...
  async findAll(filterDto) {
    ...
    const queryBuilder = this.archivoRepository
      ...
      .where('archivo.isDeleted = :isDeleted', {
        isDeleted: false,
      });
    if (usuarioId) {
  ...
  async findOne(id) {
    ...
    if (!archivo) {
      throw new _common.NotFoundException(
        _i18nhelper.I18nHelper.getError('FILE_NOT_FOUND')
      );
    }
    return archivo;
  }
  getFileContent(filename) {
    if (this.storageType === 'local') {
      ...
      if (!filePath.startsWith(uploadDirResolved + _path.sep)) {
        throw new _common.BadRequestException(
          _i18nhelper.I18nHelper.getError('INVALID_FILE_PATH')
        );
      }
      if (!_fs.existsSync(filePath)) {
        throw new _common.NotFoundException(
          _i18nhelper.I18nHelper.getError('FILE_NOT_FOUND_PHYSICAL')
        );
      }
      return filePath;
    }
    throw new _common.BadRequestException(
      _i18nhelper.I18nHelper.getError('STORAGE_TYPE_LOCAL_ONLY')
    );
  }
  async remove(id, user) {
    ...
    if (
      ...
      user.rol !== _usuarioenums.rolUsuario.ADMINISTRADOR
    ) {
      throw new _common.ForbiddenException(
        _i18nhelper.I18nHelper.getError('FILE_DELETE_FORBIDDEN')
      );
    }
...
ArchivoService = _ts_decorate(
  [
    ...
    _ts_metadata('design:paramtypes', [
      ...
      typeof _config.ConfigService === 'undefined'
        ? Object
