'use strict';
Object.defineProperty(exports, '__esModule', {
  value: true,
});
Object.defineProperty(exports, 'MovimientoModule', {
  enumerable: true,
  get: function () {
    return MovimientoModule;
  },
});
const _common = require('@nestjs/common');
const _typeorm = require('@nestjs/typeorm');
const _movimientoentity = require('./movimiento.entity/movimiento.entity');
const _movimientocontroller = require('./controller/movimiento.controller');
const _movimientoservice = require('./service/movimiento.service');
const _movimientorepository = require('./repository/movimiento.repository');
const _usuarioentity = require('../usuario/usuario.entity/usuario.entity');
function _ts_decorate(decorators, target, key, desc) {
  var c = arguments.length,
    r =
      c < 3
        ? target
        : desc === null
          ? (desc = Object.getOwnPropertyDescriptor(target, key))
          : desc,
    d;
  if (typeof Reflect === 'object' && typeof Reflect.decorate === 'function')
    r = Reflect.decorate(decorators, target, key, desc);
  else
    for (var i = decorators.length - 1; i >= 0; i--)
      if ((d = decorators[i]))
        r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
  return (c > 3 && r && Object.defineProperty(target, key, r), r);
}
let MovimientoModule = class MovimientoModule {};
MovimientoModule = _ts_decorate(
  [
    (0, _common.Module)({
      imports: [
        _typeorm.TypeOrmModule.forFeature([
          _movimientoentity.Movimiento,
          _usuarioentity.Usuario,
        ]),
      ],
      controllers: [_movimientocontroller.MovimientoController],
      providers: [
        _movimientoservice.MovimientoService,
        _movimientorepository.MovimientoRepository,
      ],
      exports: [_movimientoservice.MovimientoService],
    }),
  ],
  MovimientoModule
);
