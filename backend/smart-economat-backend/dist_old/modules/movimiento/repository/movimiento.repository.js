...
let MovimientoRepository = class MovimientoRepository {
   ...
   */ async findMovimientosByEntity(dto) {
        const { entityId, userId, type, startDate, endDate, sortBy = 'createdAt', sortOrder = 'DESC' } = dto;
        if (!entityId && !userId) {
            return [];
        }
        const query = this.repo.createQueryBuilder('movimiento').leftJoinAndSelect('movimiento.usuario', 'usuario').leftJoinAndSelect('movimiento.productoProveedor', 'productoProveedor').leftJoinAndSelect('movimiento.inventario', 'inventario');
        if (entityId) {
