...
let UsuarioService = class UsuarioService {
    ...
    async changePassword(userId, dto) {
        ...
        if (!isMatch) {
            throw new _common.UnauthorizedException(_i18nhelper.I18nHelper.getError('INVALID_OLD_PASSWORD'));
        }
    ...
    async resetPassword(id, dto) {
        ...
        if (usuario.status === _usuarioenums.UserStatus.BLOCKED) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('USER_BLOCKED_CANNOT_RESET_PASSWORD'));
        }
    ...
    async addAdditionalPermission(userId, permisoId) {
        ...
        if (!permiso) throw new _common.NotFoundException(_i18nhelper.I18nHelper.getError('PERMISO_NO_ENCONTRADO'));
    ...
    async removeAdditionalPermission(userId, permisoId) {
        ...
        basicUser.permisosAdicionales = basicUser.permisosAdicionales.filter((p)=>p.id !== permisoId);
        await this.usuarioRepo.repo.save(basicUser);
        await this.authorizationService.invalidateUserCache(userId);
        return this.findOne(userId);
    }
    ...
    async removeExcludedPermission(userId, permisoId) {
        ...
        if (!basicUser) throw new _common.NotFoundException();
        basicUser.permisosExcluidos = (basicUser.permisosExcluidos || []).filter((p)=>p.id !== permisoId);
        await this.usuarioRepo.repo.save(basicUser);
        await this.authorizationService.invalidateUserCache(userId);
        return {
            success: true
...
UsuarioService = _ts_decorate([
    ...
    _ts_metadata("design:paramtypes", [
        typeof _usuariorepository.UsuarioRepository === "undefined" ? Object : _usuariorepository.UsuarioRepository,
