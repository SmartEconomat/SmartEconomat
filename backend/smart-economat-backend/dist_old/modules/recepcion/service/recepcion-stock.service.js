...
let RecepcionStockService = class RecepcionStockService {
  async procesarRecepcionMasiva(dto, userId) {
    const usuario = await this.dataSource.manager.findOne(
    ...
    if (!pedido) {
      throw new _common.NotFoundException(
        _i18nhelper.I18nHelper.getError('ORDER_NOT_FOUND')
      );
    }
    if (
      pedido.estado !== _estadopedidoenum.EstadoPedido.PENDIENTE &&
      ...
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('ORDER_NOT_RECEPTABLE')
      );
    }
    ...
    try {
      ...
      let defaultUbicacion = await queryRunner.manager.findOne(
        _ubicacionentity.Ubicacion,
        {
          where: {
      ...
      if (!defaultUbicacion) {
        defaultUbicacion = queryRunner.manager.create(
          _ubicacionentity.Ubicacion,
          {
            nombre: 'Almacén Principal',
      ...
      let rp = queryRunner.manager.create(
      ...
      if (dto.nAlbaran) {
        let albaran = await queryRunner.manager.findOne(
        ...
        const apr = queryRunner.manager.create(
      ...
      for (const linea of dto.productosRecibidos) {
        ...
        if (linea.cantidadRecibida > 0) {
          ...
          const stockNuevo = queryRunner.manager.create(
      ...
      if (batchInventarios.length > 0) {
        ...
        for (const item of lineasConInventario) {
          batchMovimientos.push(
            queryRunner.manager.create(_movimientoentity.Movimiento, {
      ...
      for (const pp of ppArr) {
        ...
        for (const lr of lineasRecibidas) {
          if (
            lr.estadoVisual === _estadovisualenum.EstadoVisualProducto.OPTIMO
          ) {
        ...
        if (subCantRotaDefectuosa > 0) {
          const observacionesMermas = lineasRecibidas
            .filter(
              (lr) =>
                lr.estadoVisual !==
                _estadovisualenum.EstadoVisualProducto.OPTIMO
            )
      ...
      if (lineasIncidencia.length > 0) {
        recepcionEstadoEnum =
          _estadorecepcionenum.EstadoRecepcion.CON_INCIDENCIAS;
        const incidenciaRec = queryRunner.manager.create(
      ...
      const err = error;
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('RECEPTION_FAILED', {
          message: err.message,
        })
      );
    } finally {
  ...
  async procesarRecepcion(dto) {
    ...
    if (pedidoIdsList.length === 0) {
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('NO_SE_HAN_ESPECIFICADO_PEDIDOS')
      );
    }
    const uniquePedidoIds = [...new Set(pedidoIdsList)];
    const pedidosArr = await this.dataSource.manager.find(
    ...
    if (pedidosArr.length !== uniquePedidoIds.length) {
      ...
      throw new _common.NotFoundException(
        `${_i18nhelper.I18nHelper.getError('ORDER_NOT_FOUND')}: ${missingIds.join(', ')}`
      );
    }
    const pedidosInvalidos = pedidosArr.filter(
      (p) =>
        p.estado !== _estadopedidoenum.EstadoPedido.PENDIENTE &&
        p.estado !== _estadopedidoenum.EstadoPedido.EN_PROCESO &&
        p.estado !== _estadopedidoenum.EstadoPedido.PARCIAL
    );
    if (pedidosInvalidos.length > 0) {
      const invalidIds = pedidosInvalidos.map((p) => p.id);
      throw new _common.BadRequestException(
        `${_i18nhelper.I18nHelper.getError('ORDER_NOT_RECEPTABLE')}: ${invalidIds.join(', ')}`
      );
    }
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
    ...
    try {
      ...
      if (dto.productosNuevos && dto.productosNuevos.length > 0) {
        for (const pNew of dto.productosNuevos) {
          ...
          const pp = queryRunner.manager.create(
          ...
          if (pNew.cantidadRecibida > 0) {
            const inv = queryRunner.manager.create(
              _inventarioentity.Inventario,
              {
      ...
      for (const linea of dto.productos) {
        const estadoVirtualDefault =
          linea.estadoVisual || _estadovisualenum.EstadoVisualProducto.OPTIMO;
        const ppRef = mapPedidoProductos.get(linea.pedidoProductoId);
        if (!ppRef) {
          throw new _common.BadRequestException(
  ...
  async actualizarEstadoPedido(pedidoId, manager) {
    ...
    const recepcionPedidos = await manager.find(
      _recepcionpedidoentity.RecepcionPedido,
      {
        where: {
          pedido: {
