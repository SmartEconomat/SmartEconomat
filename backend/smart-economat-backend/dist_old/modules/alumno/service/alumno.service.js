...
let AlumnoService = class AlumnoService {
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (isExistingUser)
        throw new _common.ConflictException(
          _i18nhelper.I18nHelper.getError('USERNAME_OR_EMAIL_IS_ALREADY_TAKEN')
        );
  ...
  async changeProfesor(alumnoUserId, reqUserId, reqUserRole, dto) {
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!alumno)
        throw new _common.NotFoundException(
          _i18nhelper.I18nHelper.getError('ALUMNO_NO_ENCONTRADO')
        );
      ...
      if (!nuevoProfesor)
        throw new _common.NotFoundException(
          _i18nhelper.I18nHelper.getError('NUEVO_PROFESOR_NO_ENCONTRADO')
        );
      ...
      if (!nuevoSlot)
        throw new _common.NotFoundException(
          _i18nhelper.I18nHelper.getError(
            'EL_NUEVO_SLOT_ESPECIFICADO_NO_EXISTE'
          )
        );
      if (nuevoSlot.alumno)
        throw new _common.BadRequestException(
          _i18nhelper.I18nHelper.getError('EL_NUEVO_SLOT_YA_EST_OCUPADO')
        );
