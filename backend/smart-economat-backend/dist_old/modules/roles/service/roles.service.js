...
let RolesService = class RolesService {
    ...
    if (rol.esSistema) {
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError(
          'NO_SE_PUEDE_ELIMINAR_UN_ROL_DE_SISTEMA'
        )
      );
    }
   ...
   */ async assignPermissions(rolId, dto) {
    ...
    if (permisos.length !== dto.permisoIds.length) {
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('ALGUNOS_PERMISOS_NO_EXISTEN')
      );
    }
   ...
   */ async removeRoleFromUser(usuarioId, rolId) {
    ...
    if (!usuarioRol) {
      throw new _common.NotFoundException(
        _i18nhelper.I18nHelper.getError('ASIGNACI_N_DE_ROL_NO_ENCONTRADA')
      );
    }
