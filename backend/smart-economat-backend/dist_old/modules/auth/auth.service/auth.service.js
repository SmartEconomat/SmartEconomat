...
let AuthService = class AuthService {
    async register(dto) {
        return await this.dataSource.transaction(async (manager)=>{
            ...
            if (existing) {
                throw new _common.ConflictException(_i18nhelper.I18nHelper.getError('USER_OR_EMAIL_ALREADY_REGISTERED'));
            }
    ...
    async login(dto) {
        const usuario = await this.usuarioRepo.createQueryBuilder('usuario').where('(usuario.email = :email OR usuario.username = :email)', {
            email: dto.email
        }).addSelect('usuario.password').getOne();
        if (!usuario || !await _bcrypt.compare(dto.password, usuario.password)) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('INVALID_CREDENTIALS'));
        }
        if (usuario.status === _usuarioenums.UserStatus.INACTIVE) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('ACCOUNT_INACTIVE'));
        }
        if (usuario.status === _usuarioenums.UserStatus.BLOCKED) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('ACCOUNT_BLOCKED'));
        }
    ...
    async forgotPassword(email) {
        ...
        const hashedToken = _crypto.createHash('sha256').update(token).digest('hex');
    ...
    async resetPassword(token, newPassword) {
        ...
        if (!usuario || !usuario.passwordResetExpires || usuario.passwordResetExpires < new Date()) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('EL_TOKEN_ES_INV_LIDO_O_HA_EXPIRADO'));
        }
    ...
    async changePassword(userId, currentPassword, newPassword) {
        const usuario = await this.usuarioRepo.createQueryBuilder('usuario').where('usuario.id = :id', {
            id: userId
        }).addSelect('usuario.password').getOne();
        if (!usuario) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('USER_NOT_FOUND'));
        }
        if (!await _bcrypt.compare(currentPassword, usuario.password)) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('LA_CONTRASE_A_ACTUAL_ES_INCORRECTA'));
        }
        usuario.password = newPassword;
        usuario.mustChangePassword = false;
        await this.usuarioRepo.save(usuario);
