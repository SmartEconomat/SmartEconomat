...
let ProduccionService = class ProduccionService {
  async ejecutarProduccion(dto, userId) {
    ...
    if (!receta.productoResultado) {
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('RECIPE_NO_RESULT_PRODUCT')
      );
    }
    if (!receta.rendimiento || receta.rendimiento <= 0) {
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('RECIPE_NO_RENDIMIENTO')
      );
    }
    if (!receta.ingredientes || receta.ingredientes.length === 0) {
      throw new _common.BadRequestException(
        _i18nhelper.I18nHelper.getError('RECIPE_NO_INGREDIENTS')
      );
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const movimientosConsumo = consumos.map((c) =>
        manager.create(_movimientoentity.Movimiento, {
      ...
      const savedLote = await manager.findOne(
  ...
  async findAll() {
    return this.dataSource
      .getRepository(_produccionloteentity.ProduccionLote)
      .find({
        relations: ['receta', 'usuario'],
        order: {
  ...
  async findOne(id) {
    ...
    if (!lote) {
      throw new _common.NotFoundException(
        _i18nhelper.I18nHelper.getError('PRODUCCION_LOTE_NOT_FOUND')
      );
    }
    return lote;
  }
