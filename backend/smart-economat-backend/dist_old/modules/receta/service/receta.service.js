...
let RecetaService = class RecetaService {
    ...
    async duplicate(duplicateRecetaDto) {
        return this.recetaRepository.duplicate(duplicateRecetaDto.sourceId, duplicateRecetaDto.newName);
    }
    async getDetalle(id) {
        const receta = await this.recetaRepository.findById(id);
        if (!receta) {
        ...
        if (productoIds.length > 0) {
            stocks = await this.dataSource.getRepository(_inventarioentity.Inventario).createQueryBuilder('inv').innerJoin('inv.productoProveedor', 'pp').where('pp.producto_id IN (:...productoIds)', {
                productoIds
            }).select('pp.producto_id', 'productoId').addSelect('SUM(inv.cantidad_actual)', 'totalStock').groupBy('pp.producto_id').getRawMany();
        }
        ...
        return {
            receta,
            detalleIngredientes,
            alergenosConsolidados: Array.from(alergenosSet).filter((a)=>a !== null && a !== undefined)
        };
    }
    async calcularEscandallo(id) {
        ...
        const productoIds = receta.ingredientes.map((i)=>i.producto.id);
        const productosProveedores = await this.dataSource.getRepository(_productoproveedorentity.ProductoProveedor).createQueryBuilder('pp').innerJoinAndSelect('pp.producto', 'producto').leftJoinAndSelect('pp.historialPrecios', 'historial').where('producto.id IN (:...productoIds)', {
            productoIds
        }).getMany();
        const ppMap = new Map();
        ...
        for (const ing of receta.ingredientes){
            ...
            const precios = pps.map((pp)=>pp.precioUnitario).filter((p)=>p !== null && p !== undefined && p > 0);
            let precioUnitario;
            if (precios.length > 0) {
                precioUnitario = precios.reduce((sum, p)=>sum + p, 0) / precios.length;
            } else {
                const allHistorial = pps.flatMap((pp)=>pp.historialPrecios ?? []).sort((a, b)=>new Date(b.fecha).getTime() - new Date(a.fecha).getTime());
    ...
    async recalcularCostes(id) {
        ...
        const costeUnitarioEstimado = receta.rendimiento && receta.rendimiento > 0 ? costoTotal / receta.rendimiento : costoTotal;
