...
let AuthorizationService = class AuthorizationService {
   ...
   */ async getUserPermissions(userId) {
        ...
        try {
            ...
            if (cached) {
                this.logger.debug(`Permisos obtenidos desde cache para usuario ${userId}`);
                return cached;
            }
            this.logger.debug(`Cache miss - Consultando permisos desde BD para usuario ${userId}`);
   ...
   */ async loadUserPermissionsFromDB(userId) {
        ...
        if (!usuario) return [];
        const permisosRoles = await this.permisoRepo.createQueryBuilder('permiso').innerJoin('permiso.roles', 'rol').innerJoin('usuario_rol', 'ur', 'ur.rol_id = rol.id AND ur.usuario_id = :userId', {
            userId
        ...
        const permisosPlantilla = await this.permisoRepo.createQueryBuilder('permiso').innerJoin('plantilla_rol_permiso', 'prp', 'prp.permiso_id = permiso.id').innerJoin('plantilla_rol', 'plantilla', 'plantilla.id = prp.plantilla_rol_id').where('plantilla.nombre = :rolNombre', {
        ...
        const adicionales = await this.permisoRepo.createQueryBuilder('permiso').innerJoin('usuario_permiso_adicional', 'upa', 'upa.permiso_id = permiso.id').where('upa.usuario_id = :userId', {
            userId
        }).andWhere('permiso.activo = :permisoActivo', {
            permisoActivo: true
        }).select([
        ...
        const excluidos = await this.permisoRepo.createQueryBuilder('permiso').innerJoin('usuario_permiso_excluido', 'upe', 'upe.permiso_id = permiso.id').where('upe.usuario_id = :userId', {
            userId
        }).select([
            'permiso.codigo'
        ]).getMany();
   ...
   */ async invalidateUserCache(userId) {
        ...
        try {
            ...
            this.logger.error(`Error al invalidar cache del usuario ${userId}:`, error);
        }
    }
    /**
   ...
   */ async invalidateUsersCache(userIds) {
        try {
            ...
            this.logger.error(_i18nhelper.I18nHelper.translate('logs.ERROR_AL_INVALIDAR_CACHE_DE_USUARIOS'), error);
        }
    }
    /**
   ...
   */ async invalidateAllCache() {
        try {
            ...
            await cache.clear();
            this.logger.warn(_i18nhelper.I18nHelper.translate('logs.TODO_EL_CACHE_DE_PERMISOS_HA_SIDO_INVALI'));
        } catch (error) {
            this.logger.error(_i18nhelper.I18nHelper.translate('logs.ERROR_AL_INVALIDAR_TODO_EL_CACHE'), error);
        }
    }
    /**
