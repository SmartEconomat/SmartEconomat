...
let PermisosGuard = class PermisosGuard {
    async canActivate(context) {
        ...
        const methodPermissions = this.reflector.get(_requirepermissionsdecorator.PERMISSIONS_KEY, context.getHandler()) || [];
        const controllerPermissions = this.reflector.get(_requirepermissionsdecorator.PERMISSIONS_KEY, context.getClass()) || [];
        ...
        const permissionsMode = this.reflector.getAllAndOverride(_requirepermissionsdecorator.PERMISSIONS_MODE_KEY, [
            context.getHandler(),
            context.getClass()
        ]);
        ...
        if (!user || !user.id) {
            ...
            throw new _common.ForbiddenException(_i18nhelper.I18nHelper.getError('USUARIO_NO_AUTENTICADO'));
        }
        const hasPermission = permissionsMode === 'any' ? await this.authorizationService.userHasAnyPermission(String(user.id), requiredPermissions) : await this.authorizationService.userHasAllPermissions(String(user.id), requiredPermissions);
        if (!hasPermission) {
            ...
            this.logger.warn(`Usuario ${user.id} (${user.nombre || 'sin nombre'}) denegado. ${message}`);
        ...
        this.logger.debug(`Usuario ${user.id} autorizado con permisos: ${requiredPermissions.join(', ')}`);
        return true;
    }
