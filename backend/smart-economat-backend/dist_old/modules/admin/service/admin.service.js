...
let AdminService = class AdminService {
    ...
    async activateUser(userId) {
        ...
        if (!user) throw new _common.NotFoundException(_i18nhelper.I18nHelper.getError('USER_NOT_FOUND_1'));
        if (user.status === _usuarioenums.UserStatus.ACTIVE) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('USER_IS_ALREADY_ACTIVE'));
        }
        user.status = _usuarioenums.UserStatus.ACTIVE;
        await this.usuarioRepo.save(user);
        return {
            message: _i18nhelper.I18nHelper.translate('messages.USER_ACTIVATED_SUCCESSFULLY'),
            id: user.id,
            status: user.status
        };
    }
    async forcePasswordReset(userId) {
        ...
        if (!user) throw new _common.NotFoundException(_i18nhelper.I18nHelper.getError('USER_NOT_FOUND'));
        ...
        await this.usuarioRepo.save(user);
        return {
            message: _i18nhelper.I18nHelper.translate('messages.CONTRASE_A_RESTABLECIDA_EXITOSAMENTE_ENT'),
            provisionalPassword,
