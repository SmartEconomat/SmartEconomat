...
let ProductoService = class ProductoService {
    async create(createProductoDto, userId) {
        ...
        if (rest.codigoBarras) {
            if (!(0, _ean13util.validateEan13)(rest.codigoBarras)) {
                throw new _common.BadRequestException('El código de barras proporcionado no es un EAN-13 válido');
            }
            const exists = await this.productoRepository.existsByCodigoBarras(rest.codigoBarras);
            if (exists) {
                throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('EL_C_DIGO_DE_BARRAS_YA_EST_REGISTRADO'));
            }
        } else {
        ...
        return await this.dataSource.transaction(async (manager)=>{
            ...
            if (alergenos && alergenos.length > 0) {
                producto.alergenos = alergenos.map((a)=>manager.create(_productoalergenoentity.ProductoAlergeno, {
                        alergeno: a
                    }));
            }
            const savedProduct = await manager.save(producto);
            if (proveedores !== undefined) {
                await this.syncProveedoresWithManager(manager, savedProduct.id, proveedores);
            }
            await this.movimientoHelper.trackProductoCreation(userId, savedProduct.id, `Creación de producto: ${savedProduct.nombre}`);
    ...
    async findAll(query) {
        ...
        const queryBuilder = this.productoRepository.createQueryBuilder('producto').leftJoinAndSelect('producto.proveedores', 'proveedores').leftJoinAndSelect('proveedores.proveedor', 'proveedor').leftJoinAndSelect('producto.alergenos', 'alergenos');
        ...
        if (query.alergenos && query.alergenos.length > 0) {
            queryBuilder.innerJoin('producto.alergenos', 'alergenoFiltro', 'alergenoFiltro.alergeno IN (:...alergenos)', {
                alergenos: query.alergenos
            });
        }
        if (query.minStock) {
            queryBuilder.innerJoin('proveedores.inventarios', 'inventarios', 'inventarios.cantidad_actual > 0');
        }
    ...
    async update(id, updateProductoDto, userId) {
        ...
        await this.movimientoHelper.trackProductoUpdate(userId, id, `Actualización de producto: ${producto.nombre}`);
    ...
    async remove(id, userId) {
        ...
        await this.movimientoHelper.trackProductoDeletion(userId, id, `Eliminación de producto: ${producto.nombre}`);
    }
    async generateUniqueEan13() {
        ...
        throw new _common.InternalServerErrorException('No se pudo generar un código EAN-13 único después de varios intentos');
    }
    async syncProveedoresWithManager(manager, productoId, proveedores) {
        const existing = await manager.find(_productoproveedorentity.ProductoProveedor, {
            where: {
                producto: {
                    id: productoId
                }
        ...
        const newProveedores = proveedores.filter((p)=>!existingIds.includes(p.proveedorId));
        const proveedoresToUpdate = proveedores.filter((p)=>existingIds.includes(p.proveedorId));
        if (newProveedores.length > 0) {
            const newRelations = newProveedores.map((p)=>manager.create(_productoproveedorentity.ProductoProveedor, {
                    producto: {
                        id: productoId
                    },
                    proveedor: {
        ...
        const idsToRemove = existingIds.filter((id)=>!currentProviderIds.includes(id));
        if (idsToRemove.length > 0) {
            for (const id of idsToRemove){
                const toDelete = existing.find((e)=>e.proveedor.id === id);
                if (toDelete) {
                    await manager.softDelete(_productoproveedorentity.ProductoProveedor, toDelete.id);
                }
            }
        }
    }
    async syncProveedores(productoId, proveedores) {
        return this.syncProveedoresWithManager(this.dataSource.manager, productoId, proveedores);
    }
    constructor(productoRepository, productoProveedorRepository, productoAlergenoRepository, movimientoHelper, dataSource){
...
ProductoService = _ts_decorate([
    ...
    _ts_param(2, (0, _typeorm.InjectRepository)(_productoalergenoentity.ProductoAlergeno)),
    _ts_param(4, (0, _typeorm.InjectDataSource)()),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", [
        typeof _productorepository.ProductoRepository === "undefined" ? Object : _productorepository.ProductoRepository,
