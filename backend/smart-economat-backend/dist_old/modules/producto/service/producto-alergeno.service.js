...
let ProductoAlergenoService = class ProductoAlergenoService {
   ...
   */ async create(dto) {
    ...
    if (existing) {
      throw new _common.ConflictException(
        _i18nhelper.I18nHelper.getError('PRODUCTO_ALERGENO_ALREADY_EXISTS')
      );
    }
   ...
   */ async update(idProducto, dto) {
    ...
    await this.productoAlergenoRepository
      .createQueryBuilder()
      .delete()
      .from(_productoalergenoentity.ProductoAlergeno)
   ...
   */ async remove(idProducto, alergeno) {
    ...
    if (!productoAlergeno) {
      throw new _common.NotFoundException(
        _i18nhelper.I18nHelper.getError('PRODUCTO_ALERGENO_NOT_FOUND')
      );
    }
    await this.productoAlergenoRepository
      .createQueryBuilder()
