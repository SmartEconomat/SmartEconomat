...
let ProfesorService = class ProfesorService {
    async register(dto) {
        return this.dataSource.transaction(async (manager)=>{
            ...
            if (isExisting) throw new _common.ConflictException(_i18nhelper.I18nHelper.getError('USER_OR_EMAIL_ALREADY_EXISTS'));
            const isCialExisting = await manager.findOne(_profesorentity.Profesor, {
                where: {
                    cial: dto.cial
                }
            });
            if (isCialExisting) throw new _common.ConflictException(_i18nhelper.I18nHelper.getError('CIAL_ALREADY_EXISTS'));
            ...
            return {
                message: _i18nhelper.I18nHelper.translate('messages.PROFESOR_REGISTRADO_CON_XITO_ESPERANDO_A'),
                id: profesor.id,
    ...
    async createSlot(userId, dto) {
        ...
        if (!profesor) {
            throw new _common.NotFoundException(_i18nhelper.I18nHelper.getError('PROFESSOR_PROFILE_NOT_FOUND'));
        }
        ...
        if (existingSlot) {
            throw new _common.ConflictException('Slot already exists for this aula and numeroClase');
        }
    ...
    async activateAlumno(profesorUserId, alumnoId) {
        return this.dataSource.transaction(async (manager)=>{
            ...
            if (!profesor) throw new _common.NotFoundException(_i18nhelper.I18nHelper.getError('PROFESOR_NOT_FOUND'));
            ...
            if (!alumno) throw new _common.NotFoundException('Alumno no pertenece a este profesor o no existe');
            alumno.user.status = _usuarioenums.UserStatus.ACTIVE;
            await manager.save(alumno.user);
            return {
                message: _i18nhelper.I18nHelper.translate('messages.ALUMNO_ACTIVADO_CORRECTAMENTE')
            };
        });
    }
    async getAlumnos(profesorUserId) {
        ...
        if (!profesor) throw new _common.NotFoundException(_i18nhelper.I18nHelper.getError('PROFESOR_NOT_FOUND'));
        const result = await this.dataSource.getRepository(_alumnoentity.Alumno).find({
            where: {
                slot: {
                    profesor: {
                        id: profesor.id
    ...
    async forcePasswordReset(profesorUserId, alumnoId) {
        ...
        if (!profesor) throw new _common.NotFoundException(_i18nhelper.I18nHelper.getError('PROFESOR_NOT_FOUND'));
        const alumno = await this.dataSource.getRepository(_alumnoentity.Alumno).findOne({
            where: {
                id: alumnoId,
                slot: {
                    profesor: {
        ...
        if (!alumno) {
            throw new _common.NotFoundException('Alumno no pertenece a este profesor o no existe');
        }
        ...
        await this.dataSource.getRepository(_usuarioentity.Usuario).save(alumno.user);
        return {
            message: _i18nhelper.I18nHelper.translate('messages.CONTRASE_A_RESTABLECIDA_EXITOSAMENTE_ENT_1'),
            provisionalPassword,
