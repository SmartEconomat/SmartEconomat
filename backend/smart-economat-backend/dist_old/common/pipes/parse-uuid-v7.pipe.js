...
let ParseUUIDv7Pipe = class ParseUUIDv7Pipe {
   ...
   */ transform(value) {
        if (!value) {
            throw new _common.BadRequestException(_i18nhelper.I18nHelper.getError('EL_UUID_NO_PUEDE_ESTAR_VAC_O'));
        }
        if (!(0, _classvalidator.isUUID)(value, '7')) {
            throw new _common.BadRequestException(`El valor '${value}' no es un UUID v7 válido`);
        }
        if (!ParseUUIDv7Pipe.UUID_V7_REGEX.test(value)) {
            throw new _common.BadRequestException(`El valor '${value}' no tiene el formato correcto de UUID v7`);
        }
        return value;
    }
};
/**
   ...
   */ ParseUUIDv7Pipe.UUID_V7_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
ParseUUIDv7Pipe = _ts_decorate([
    (0, _common.Injectable)()
], ParseUUIDv7Pipe);

//# sourceMappingURL=parse-uuid-v7.pipe.js.map