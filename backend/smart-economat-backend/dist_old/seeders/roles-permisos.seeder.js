...
_export(exports, {
    ...
    get seedRolesPermisos () {
        return seedRolesPermisos;
    }
});
const _permisoentity = require("../modules/permisos/entities/permiso.entity");
const _plantillarolentity = require("../modules/plantillas-roles/entities/plantilla-rol.entity");
...
async function seedRolesPermisos(dataSource) {
    const permisoRepo = dataSource.getRepository(_permisoentity.Permiso);
    const plantillaRepo = dataSource.getRepository(_plantillarolentity.PlantillaRol);
    ...
    console.log(`✅ ${permisosCreados.length} permisos creados (${PERMISOS_BASE.length} totales en BD)`);
    ...
    if (!superAdmin) {
        ...
        console.log(`✅ Plantilla SUPER_ADMIN creada (${todosPermisos.length} permisos)`);
    }
    let administrador = await plantillaRepo.findOne({
        where: {
            nombre: _usuarioenums.rolUsuario.ADMINISTRADOR
        }
    });
    if (!administrador) {
        const permisosAdmin = todosPermisos.filter((p)=>!p.codigo.startsWith('roles:') && !p.codigo.startsWith('permisos:') || p.codigo === 'permisos:gestionar');
        administrador = plantillaRepo.create({
            nombre: _usuarioenums.rolUsuario.ADMINISTRADOR,
            descripcion: 'Administrador completo del economato (incluye gestión de permisos de usuario)',
            esEditable: true,
            activo: true
        });
        ...
        console.log(`✅ Plantilla ADMINISTRADOR ('${_usuarioenums.rolUsuario.ADMINISTRADOR}') creada (${permisosAdmin.length} permisos)`);
    }
    let gestor = await plantillaRepo.findOne({
        where: {
            nombre: _usuarioenums.rolUsuario.PROFESOR
        }
    });
    if (!gestor) {
        const permisosGestor = todosPermisos.filter((p)=>[
                'productos',
                'pedidos',
                'recepciones',
                'inventario',
        ...
        console.log(`✅ Plantilla PROFESOR creada (${permisosGestor.length} permisos)`);
    }
    let usuarioBasico = await plantillaRepo.findOne({
        where: {
            nombre: _usuarioenums.rolUsuario.ALUMNO
        }
    });
    if (!usuarioBasico) {
        const permisosBasico = todosPermisos.filter((p)=>[
                'productos',
                'inventario',
                'dashboard',
                'albaranes',
                ...
                'ver',
                'ver_estadisticas'
            ].includes(p.accion) || p.modulo === 'alumno');
        ...
        console.log(`✅ Plantilla ALUMNO creada (${permisosBasico.length} permisos)`);
    }
    console.log('🎉 Seeder de roles y permisos completado exitosamente');
}
async function runSeeder(dataSource) {
    await seedRolesPermisos(dataSource);
}

//# sourceMappingURL=roles-permisos.seeder.js.map