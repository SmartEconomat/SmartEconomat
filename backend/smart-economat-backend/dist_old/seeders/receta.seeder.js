...
const runSeeder = async (dataSource) => {
  ...
  const ingredienteRepo = dataSource.getRepository(
  ...
  if (productos.length === 0) {
    throw new Error(
      _seederi18nhelper.SeederI18nHelper.getError('NO_PRODUCTOS')
    );
  }
  for (let i = 0; i < NUM_RECETAS; i++) {
    const receta = recetaRepo.create({
      ...
      tiempo: faker.helpers.arrayElement(
        Object.values(_recetaenums.TiempoReceta)
      ),
      dificultad: faker.helpers.arrayElement(
        Object.values(_recetaenums.DificultadReceta)
      ),
    ...
    const ingredientes = productosAleatorios.map((producto) =>
      ingredienteRepo.create({
        ...
        unidad: faker.helpers.arrayElement(
          Object.values(_recetaenums.UnidadIngrediente)
        ),
      })
    );
