...
const dataSource = new _typeorm.DataSource({
  ...
  migrations: [(0, _path.join)(__dirname, '../migrations/*.{ts,js}')],
  synchronize:
...
async function runAllSeeders() {
  ...
  for (const name of seedersInOrder) {
    ...
    if (typeof seeder.runSeeder === 'function') {
      console.log(
        _seederi18nhelper.SeederI18nHelper.getSeederMessage('running', {
          file: filePath,
        })
      );
...
async function runSeederByName(name) {
  ...
  if (!filePath) {
    throw new Error(
      `${_seederi18nhelper.SeederI18nHelper.getError('SEEDER_NOT_FOUND')}: ${name}`
    );
  }
  const seeder = require((0, _path.join)(__dirname, filePath));
  if (typeof seeder.runSeeder !== 'function') {
    throw new Error(
      `${_seederi18nhelper.SeederI18nHelper.getError('RUN_SEEDER_NOT_FOUND')} ${filePath}`
    );
  }
  console.log(
    _seederi18nhelper.SeederI18nHelper.getSeederMessage('running', {
