...
const runSeeder = async (dataSource) => {
  ...
  const incidenciaLineaRepo = dataSource.getRepository(
    _incidencialineaentity.IncidenciaLinea
  );
  const recepcionPedidoRepo = dataSource.getRepository(
  ...
  if (recepcionPedidos.length === 0) {
    throw new Error(
      _seederi18nhelper.SeederI18nHelper.getError('NO_RECEPCIONES')
    );
  }
  const incidencias = [];
  ...
  console.log(
    _seederi18nhelper.SeederI18nHelper.getSeederSuccess('incidencias', {
