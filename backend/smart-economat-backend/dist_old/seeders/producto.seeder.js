...
function mapTipoCategoria(tags = []) {
  ...
  if (tagStr.includes('sugar') || tagStr.includes('sweet'))
    return _productoenums.TipoProducto.AZUCAR;
  if (tagStr.includes('condiment') || tagStr.includes('sauce'))
    return _productoenums.TipoProducto.CONDIMENTO;
...
const runSeeder = async (dataSource) => {
  ...
  const productoAlergenoRepo = dataSource.getRepository(
    _productoalergenoentity.ProductoAlergeno
  );
  const proveedores = await proveedorRepo.find();
  if (proveedores.length === 0) {
    throw new Error(
      _seederi18nhelper.SeederI18nHelper.getError('NO_PROVEEDORES')
    );
  }
