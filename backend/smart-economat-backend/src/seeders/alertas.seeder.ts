/**
 * @module seeders/alertas
 * HTTP seeder that populates stock alert records via the REST API.
 */
import { SeedContext } from './seed-context';
import { runNamedHttpSeeder } from './http-seed.catalog';

/**
 * Seeds alert data through the named HTTP seeder catalog.
...
export const runSeeder = async (context: SeedContext) => {
  await runNamedHttpSeeder('alertas', context);
};
