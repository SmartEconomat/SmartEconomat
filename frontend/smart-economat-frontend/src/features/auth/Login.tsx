...
export default function Login() {
    ...
    const formAnim = animated ? (isLogin ? 'formPeelToRight' : 'formPeelToLeft') : 'none';
    const slideAnim = animated ? (isLogin ? 'slidePeelToLeft' : 'slidePeelToRight') : 'none';
    const peelCss = (name: string) =>
        name === 'none' ? 'none' : `${name} ${PEEL_DURATION}ms ${PEEL_EASE} both`;

    // ── Layout exit animation name ──
    const layoutAnim = phase === 'login-exit'
    ...
    const showOverlay = phase === 'login-exit' || phase === 'register-exit' || phase === 'register-return';
    const overlayIsRegister = phase === 'register-exit' || phase === 'register-return';
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    >
                        {isLogin
                            ? <LoginForm onToggleForm={toggleForm} onLoginSuccess={handleLoginSuccess} />
                            : <RegisterForm onToggleForm={toggleForm} onRegisterSuccess={handleRegisterSuccess} />
                        }
                    </Box>
                </Box>
            </Box>
            ...
            {showOverlay && (
                ...
                >
                    <Box
                        sx={{
                            textAlign: 'center',
                            animation: 'overlayContentPop 0.55s cubic-bezier(0.34,1.56,0.64,1) 0.2s both',
                        }}
                    >
                        {overlayIsRegister ? (
                            <>
