      ...
      switch (event.key) {
        case 'F1':
          ...
          if (navigateToElement('main-content')) {
            toast.info('Navegando al contenido principal (F1)', 2000, { iconType: 'navigation' });
          }
          break;
        case 'F2':
          event.preventDefault();
          if (navigateToElement('sidebar-nav')) {
            toast.info('Navegando al menú lateral (F2)', 2000, { iconType: 'navigation' });
          }
          break;
        case 'F3':
          event.preventDefault();
          if (navigateToElement('filters-area')) {
            toast.info('Navegando a la zona de filtros (F3)', 2000, { iconType: 'navigation' });
          }
          break;
        case 'F4':
          event.preventDefault();
          if (navigateToElement('results-area')) {
            toast.info('Navegando a los resultados (F4)', 2000, { iconType: 'navigation' });
          }
          break;
        default:
          break;
