    ...
    return (
        <Dialog open={open} onClose={onClose}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 400 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 2 }}>
                        <TextField
                            ...
                            onChange={e => setModalData({ ...modalData, unidad: e.target.value as any })}
                            fullWidth
                        >
                            {Object.values(UnidadMedida).map(u => (
                                <MenuItem key={u} value={u}>{u.toUpperCase()}</MenuItem>
                            ))}
                        </TextField>
                        <TextField
                            label="Categoría"
                            ...
                            onChange={e => setModalData({ ...modalData, tipo: e.target.value as any })}
                            fullWidth
                        >
                    ...
                    <TextField
                        ...
                        onChange={e => setModalData({ ...modalData, contenido: Math.max(0, Number(e.target.value) || 0) })}
                        fullWidth
                    />
                </Box>
            </DialogContent>
            <DialogActions>
                ...
                <Button variant="contained" onClick={onConfirm} disabled={!modalData.nombre}>
                    Confirmar y Añadir
                </Button>
            </DialogActions>
        </Dialog>
