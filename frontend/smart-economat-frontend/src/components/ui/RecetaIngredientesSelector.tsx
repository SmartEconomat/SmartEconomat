...
import { RecetaIngrediente, UnidadIngrediente } from '../../services/receta.types';
...
const RecetaIngredientesSelector: React.FC<RecetaIngredientesSelectorProps> = ({ value = [], onChange }) => {
    ...
    const handleAddLine = () => {
        const newLines = [...value, { productoId: '', cantidad: 1, unidad: UnidadIngrediente.GRAMO }];
        onChange(newLines);
    };
    ...
    const handleUpdateLine = (index: number, field: keyof UI_RecetaIngrediente, newValue: any) => {
    ...
    return (
        <Box sx={{ mt: 3 }}>
            ...
            {uniqueAllergens.length > 0 && (
                <Box sx={{ mb: 2, p: 2, bgcolor: 'warning.light', borderRadius: 1, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', color: 'warning.contrastText', gap: 1 }}>
                        <ErrorOutlineIcon />
                    ...
                    <Table size="small">
                        <TableHead sx={{ bgcolor: 'action.hover' }}>
                            <TableRow>
                                ...
                                <TableCell sx={{ fontWeight: 'bold', width: 150 }}>Unidad</TableCell>
                        ...
                        <TableBody>
                            {value.length === 0 ? (
                                ...
                                value.map((line, index) => {
                                    ...
                                    if (line.producto && !allProducts.find(p => p.id === line.producto?.id)) {
                                        ...
                                        <TableRow key={index}>
                                            <TableCell>
                                                <Autocomplete
                                                    ...
                                                    onChange={(_, newValue) => handleUpdateLine(index, 'productoId', newValue?.id || '')}
                                                    renderInput={(params) => (
                                            ...
                                            <TableCell>
                                                <Select
                                                    value={line.unidad || ''}
                                                    onChange={(e) => handleUpdateLine(index, 'unidad', e.target.value)}
                                                    variant="standard"
                                                    fullWidth
                                                >
