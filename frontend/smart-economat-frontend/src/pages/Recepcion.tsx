...
const Recepcion: React.FC = () => {
  ...
  const processProductFound = (prod: any) => {
    ...
    if (matches.length > 0) {
      ...
      if (existingEsp) {
        ...
        setDraft({
          ...draft,
          productosEspontaneos: [...draft.productosEspontaneos, newLinea],
        });
      }
    }
  };
            ...
            {pedidosDisponibles.map((pedido) => (
              <ListItem key={pedido.id} divider disablePadding>
                <ListItemButton
                  ...
                  selected={draft.pedidosSeleccionados.some(
                    (p) => p.id === pedido.id
                  )}
                >
                  <Checkbox
                    checked={draft.pedidosSeleccionados.some(
                      (p) => p.id === pedido.id
                    )}
                  />
                  <ListItemText
      ...
      {draft.pedidosSeleccionados.map((p, pIdx) => (
        <Accordion
          ...
          onChange={(e, isExpanded) =>
            setExpandedPanel(isExpanded ? p.id : false)
          }
  ...
  const renderStep3 = () => (
    <Box>
        ...
        >
          ...
          <AccordionDetails sx={{ p: 0 }}>
            <TableContainer
              component={Paper}
              variant="outlined"
              sx={{ mt: 1, border: 'none', boxShadow: 'none' }}
            >
              <Table size="small" sx={{ tableLayout: 'fixed' }}>
                <TableHead>
                  <TableRow>
      ...
      {draft.productosEspontaneos.length > 0 && (
        <Paper
          sx={{ p: 2, mb: 2, bgcolor: '#fafafa', border: '1px solid #eee' }}
          elevation={0}
        >
          <Typography variant="subtitle2" color="secondary">
            Especial / Fuera de Pedido 🆕
          ...
          <Box display="flex" gap={4}>
            ...
            {resultado?.productosCreados &&
              resultado.productosCreados.length > 0 && (
                <Box>
                  <Typography variant="h4">
                    {resultado.productosCreados.length}
                  </Typography>
          ...
          >
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <TextField
                ...
                onChange={(e) =>
                  setModalData({ ...modalData, unidad: e.target.value as any })
                }
                fullWidth
              >
              ...
              <TextField
                ...
                onChange={(e) =>
                  setModalData({ ...modalData, tipo: e.target.value as any })
                }
                fullWidth
              >
