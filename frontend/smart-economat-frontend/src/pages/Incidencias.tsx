...
import { Box, Paper, Typography, Alert, IconButton, Tooltip, alpha, useTheme, Button, Stack, Divider } from '@mui/material';
...
import { Incidencia, IncidenciasQueryParams } from '../services/incidencia.types';
import { fetchIncidencias, resolveIncidencia, removeIncidencia } from '../services/incidencia.service';
import { useToast } from '../store/ToastContext';
import ConfirmDialog from '../components/ui/ConfirmDialog';
import IncidenciaFilters, { IncidenciaFiltersState } from '../features/incidencias/IncidenciaFilters';
...
const Incidencias: React.FC = () => {
    ...
    const handleResolve = async (id: string, observaciones: string) => {
        setIsResolving(true);
        try {
            await resolveIncidencia(id, { observacionesResolucion: observaciones });
        ...
        },
        {
            ...
            render: (row) => (
                <StatusChip
                    ...
                    icon={row.resuelta ? <CheckCircleIcon /> : <ReportProblemOutlinedIcon />}
                />
            )
        }
    ], []);
    ...
    const renderActions = (row: Incidencia) => (
        <Stack direction="row" spacing={0.5} sx={{ minWidth: 120, justifyContent: 'flex-start' }}>
            <Box sx={{ width: 34, display: 'flex', justifyContent: 'center' }}>
                <Tooltip title="Ver detalle">
                    <IconButton onClick={() => setItemToView(row)} size="small" color="inherit">
            ...
            <Box sx={{ width: 34, display: 'flex', justifyContent: 'center' }}>
                {!row.resuelta && (
                    <Tooltip title="Marcar como resuelta">
                        <IconButton onClick={() => setItemToResolve(row)} size="small" color="success">
            ...
            <Box sx={{ width: 34, display: 'flex', justifyContent: 'center' }}>
                <Tooltip title="Eliminar">
                    <IconButton onClick={() => setItemToDelete(row)} size="small" color="error">
        ...
        return [
            {
                ...
                fields: [
                    ...
                    { label: 'Fecha de Registro', value: new Date(itemToView.createdAt).toLocaleString() },
                    { label: 'Estado', value: itemToView.resuelta ? 'Resuelta' : 'Pendiente' },
            ...
            },
            {
                ...
                fields: [
                    { label: 'Notas', value: itemToView.observacionesRecepcion || 'Sin observaciones', fullWidth: true },
                ]
            },
            ...(itemToView.resuelta ? [{
                title: 'Resolución',
                    ...
                    <Box sx={{
                        ...
                        <Stack spacing={2}>
                            <Box>
                                ...
                                <Typography variant="body2" fontWeight={600}>
                                    {itemToView.fechaResolucion ? new Date(itemToView.fechaResolucion).toLocaleString() : '—'}
                                </Typography>
                            </Box>
                            <Box>
                                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                        ...
                        {itemToView.lineas?.map((linea, idx) => (
                            <Box key={linea.id} sx={{
                                ...
                                <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1}>
                                    ...
                                    <StatusChip
                                        status={linea.tipoDiferencia === 'FALTANTE' ? 'error' : 'warning'}
                                ...
                                <Stack direction="row" spacing={3}>
                                    <Box>
                                        ...
                                        <Typography variant="body2" fontWeight={600}>{linea.cantidadEsperada}</Typography>
                                    </Box>
                                    <Box>
                                        <Typography variant="caption" color="text.secondary">Recibido</Typography>
                                        <Typography variant="body2" fontWeight={600}>{linea.cantidadRecibida}</Typography>
                                    </Box>
                                    <Box>
                                        <Typography variant="caption" color="text.secondary">Diferencia</Typography>
                                        <Typography variant="body2" fontWeight={600} color="error.main">
                                            {linea.diferencia > 0 ? `+${linea.diferencia}` : linea.diferencia}
                                        </Typography>
                                    </Box>
                                </Stack>
                                {linea.observaciones && (
            ...
            <Paper elevation={0} sx={{ p: { xs: 2, sm: 4 }, borderRadius: 2 }}>
                ...
                <DataTable
                    ...
                    pagination={{
                        ...
                        pageSize: pageSize,
                        onPageSizeChange: (e) => { setPageSize(Number(e.target.value)); setPage(1); }
                    }}
                />
            </Paper>
