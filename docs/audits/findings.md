...
| L-006 | `src/modules/auth/mail.service.ts` | 11 | **`URL_FRONTEND_DERIVADA` con fallback a `localhost:5173` en producción.** Si la variable no está definida, los correos de recuperación de contraseña apuntarán a localhost, inutilizables para el receptor. | `process.env.URL_FRONTEND_DERIVADA \|\| 'http://localhost:5173'` | Validar que `URL_FRONTEND_DERIVADA` esté definido en producción o lanzar advertencia al inicializar `MailService`. |
