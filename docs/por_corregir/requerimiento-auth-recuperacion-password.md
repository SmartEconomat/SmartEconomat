...
- La configuracion de correo y entorno esta documentada en [../environment-variables.md](../environment-variables.md).
- El backend soporta recuperación de contraseña mediante `POST /auth/forgot-password` y `POST /auth/reset-password`.
- El envío real de correo depende de que existan variables SMTP (`MAIL_HOST`, `MAIL_USER`, `MAIL_PASS`, `EMAIL_DERIVADO`). Sin esa configuración, el sistema opera en modo simulación y registra el intento en logs.
